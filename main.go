...
func startServerNew(registryAddr string, wg *sync.WaitGroup) {
	var foo Foo
	...
	// registry.HeartBeat(registryAddr, "tcp@"+hl.Addr().String(), 0)
	wg.Done()
...
func newBroadcast(registry string) {
	d := xclient.NewGoRegistryDiscovery(registry, 0)
	xc := xclient.NewXClient(d, xclient.WeightRoundRobinSelect, nil)
	defer func() { _ = xc.Close() }()
	var wg sync.WaitGroup
...
func main() {
	...
	time.Sleep(time.Second)
	wg.Add(2)
	go startServerNew(registryAddr, &wg)
	go startServerNew(registryAddr, &wg)
	wg.Wait()

	time.Sleep(time.Second)
	newCall(registryAddr)
	newBroadcast(registryAddr)
